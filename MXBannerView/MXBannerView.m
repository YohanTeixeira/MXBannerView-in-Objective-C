...
- (UIColor *)selectedDotColor {
    if (_selectedDotColor == nil) {
        _selectedDotColor = [UIColor blueColor];
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if ([self pageControl]) {
        ...
        NSUInteger currentIndex = offsetIndex % [[[self imageBox] imageArray] count] + 5;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self imageBox]) {
        NSUInteger actualItemIndex = [indexPath item] % ([self actualItemCount] / kMXImageTimes);
